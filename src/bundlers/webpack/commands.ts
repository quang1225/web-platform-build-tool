...
import loadConfig from "./webpack.config";
...
const dumpConfiguration = async () => {
  ...
  console.log(
    chalk.green(`\nWebpack configuration dumped to: ${dumpFilePath}`)
  );
};
...
const start = async (options?: CommandOptions) => {
  ...
  runNodeCommand(
    `NODE_ENV=development && ${cliPaths.cliNodeModulesBin}/webpack serve --config=${webpackConfigPath} --mode=development --progress`
  );
};
...
const build = async (options?: CommandOptions) => {
  ...
  console.log(chalk.cyan("\nBuilding...\n"));
  runNodeCommand(
    `NODE_ENV=production && ${cliPaths.cliNodeModulesBin}/webpack --config=${webpackConfigPath} --mode=production --progress`
  );
};
